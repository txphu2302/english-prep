...
import { Clock, ChevronLeft, ChevronRight, Check, Menu, Flag, AlertCircle, BookOpen, ListChecks, Eye, CheckCircle2 } from 'lucide-react';
...
const mockQuestions = [
  {
    ...
    questionNumber: 1,
    type: 'matching',
    instruction: 'Look at the following theories (Questions 1-8) and the list of researchers below. Match each theory with the correct researcher(s), A-F.',
  ...
  // Passage 3 questions (starting from 27)
  {
    ...
    questionNumber: 27,
    type: 'matching',
    instruction: 'Look at the following claims (Questions 27-30) and the list of options below. Match each claim with the correct option, A-G.',
...
export function TestInterface({ session, onComplete, onUpdateSession }: TestInterfaceProps) {
  ...
  const handlePreviousQuestion = () => {
    if (currentQuestionIndex > 0) {
      ...
      setCurrentPassageIndex(currentPassageIndex - 1);
      const prevPassageQuestions = mockQuestions.filter(q => q.passageId === mockPassages[currentPassageIndex - 1].id);
  ...
  const handleCompleteTest = () => {
    ...
    const correctAnswersMap: Record<string, string> = {
      'q1': 'B', 'q2': 'A', 'q3': 'E', 'q4': 'F', 'q5': 'A', 'q6': 'C', 'q7': 'C', 'q8': 'A',
      'q9': 'FALSE', 'q10': 'TRUE', 'q11': 'FALSE', 'q12': 'NOT GIVEN', 'q13': 'TRUE',
      'q14': 'C', 'q15': 'D', 'q16': 'E', 'q17': 'F', 'q18': 'G', 'q19': 'A', 'q20': 'B',
      'q21': 'dry dusty hole', 'q22': '300 metres', 'q23': '46000 50000', 'q24': 'megafauna', 'q25': 'fish',
      'q26': 'A temple', 'q27': 'Third century BCE', 'q28': 'Bronze statues', 'q29': '1683', 'q30': 'First national public museum',
      'q31': 'Renaissance', 'q32': 'Rome',
      'q33': 'FALSE', 'q34': 'TRUE', 'q35': 'NOT GIVEN', 'q36': 'FALSE', 'q37': 'FALSE', 'q38': 'TRUE', 'q39': 'TRUE', 'q40': 'TRUE'
    };
    ...
    const finalSession = {
      ...
      questions: mockQuestions.map((q, index) => ({
        ...
        options: q.type === 'matching' ? q.researchers : 
                 q.type === 'multiple-choice' && q.options ? q.options :
                 q.type === 'true-false' ? ['TRUE', 'FALSE', 'NOT GIVEN'] : 
                 undefined,
  ...
  const isLastQuestion = currentPassageIndex === mockPassages.length - 1 && 
      ...
      <div className="fixed inset-0 flex flex-col bg-background">
        ...
        <div className="border-b bg-white dark:bg-gray-950 shadow-sm">
          ...
          <div className="px-3 sm:px-6 py-2 sm:py-3 flex items-center justify-between gap-2">
            ...
            {/* Timer */}
            <div className={`flex items-center gap-2 sm:gap-3 px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg border ${getTimerBgColor()} transition-colors shrink-0`}>
              ...
              <div>
                ...
                <div className={`font-mono text-sm sm:text-base ${getTimerColor()}`}>
                  {formatTime(timeRemaining)}
                </div>
              </div>
              {timeRemaining <= 300 && (
                <AlertCircle className="h-4 w-4 sm:h-5 sm:w-5 text-red-600 animate-pulse" />
              )}
            </div>
          </div>
                  ...
                  {currentPassage.title && (
                    <div>
                      ...
                      {currentPassage.subtitle && (
                        <p className="text-xs sm:text-sm text-white/80 mt-1 italic line-clamp-1">{currentPassage.subtitle}</p>
                      )}
                    </div>
                  )}
                </div>
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2">
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          ...
                          >
                            <Flag className={`h-3.5 w-3.5 sm:h-4 sm:w-4 ${flaggedQuestions.has(currentQuestion?.id) ? 'fill-current' : ''}`} />
                          </Button>
                        </TooltipTrigger>
                        <TooltipContent>
                        ...
                        <CardContent className="p-3 sm:p-4">
                          <h4 className="mb-2 sm:mb-3 font-semibold text-sm sm:text-base">
                            {currentSection.questionRange === '1-8' ? 'List of Researchers' : 
                             currentSection.questionRange === '14-19' ? 'List of Options' : 'Options'}
                          </h4>
                      ...
                      <CardContent className="p-4 sm:p-6 space-y-4 sm:space-y-6">
                        ...
                        <div className="flex gap-2 sm:gap-3">
                          <div className="flex items-start gap-2 sm:gap-3 flex-1">
                            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
                              <span className="font-semibold text-primary text-xs sm:text-sm">{currentQuestion?.questionNumber}</span>
                            </div>
                            <span className="pt-0.5 sm:pt-1 text-sm sm:text-base leading-relaxed">{currentQuestion?.text}</span>
                          </div>
                        </div>
                        ...
                        {currentQuestion?.type === 'matching' && currentSection?.options && (
                          <div className="space-y-4">
                            ...
                            <div className="flex items-center justify-between text-xs text-muted-foreground pt-2">
                              <span>Choose from options {currentSection.options.map(o => o.charAt(0)).join('-')}</span>
                              {answers[currentQuestion.id] && (
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="text-xs text-muted-foreground bg-muted/50 p-3 rounded-lg">
                              <div className="space-y-1">
                                <p><strong>TRUE:</strong> Thông tin khớp với đoạn văn</p>
                                <p><strong>FALSE:</strong> Thông tin trái với đoạn văn</p>
                                <p><strong>NOT GIVEN:</strong> Không có thông tin trong đoạn văn</p>
                              </div>
                            </div>
                          </div>
                        )}
                        ...
                        {currentQuestion?.type === 'fill-blank' && (
                          <div className="space-y-2 sm:space-y-3">
                            <Label htmlFor="answer" className="text-sm sm:text-base">Your answer:</Label>
                            <Input
                              id="answer"
                              type="text"
                      ...
                      <div className="space-y-1.5 sm:space-y-2">
                        {passageQuestions.map((q, idx) => {
                          ...
                          return (
                            ...
                            >
                              <div className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center shrink-0 text-xs sm:text-sm ${
                                isAnswered ? 'bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 font-semibold' : 'bg-gray-100 dark:bg-gray-800'
                              }`}>
                                {q.questionNumber}
                              </div>
