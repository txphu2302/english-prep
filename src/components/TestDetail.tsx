		...
		<div className='max-w-6xl mx-auto p-6 space-y-6'>
			...
			<div className='space-y-4'>
				...
				{activeTab === 'fulltest' && (
					<div className='bg-blue-50 border border-blue-200 rounded-lg p-4 flex gap-3 text-blue-800 text-sm'>
						...
						<p>
							<strong>Lưu ý:</strong> Để được quy đổi sang scaled score (ví dụ trên thang điểm 9.0 cho IELTS hoặc 990
							cho TOEIC), vui lòng chọn chế độ làm <strong>Full Test</strong> với thời gian chuẩn.
						</p>
					</div>
				)}
			</div>
			...
			<div className='space-y-6'>
				...
				{activeTab === 'practice' && (
					<div className='space-y-6'>
						<div>
							<h3 className='text-xl font-semibold text-gray-900 mb-4'>Chọn phần thi bạn muốn luyện tập</h3>
							...
							<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
								{rootSections.map((section, index) => {
									...
									return (
										<Card
											key={section.id}
											className={`cursor-pointer transition-all hover:shadow-lg border-2 p-4 ${
							...
							<CardContent className='p-6'>
								<div className='flex items-start gap-4'>
									...
									<div className='flex-1'>
										<h3 className='text-xl font-semibold text-gray-900 mb-2'>Chế độ Full Test</h3>
													...
													<div className='flex-1'>
														<div className='flex items-center gap-2 mb-2'>
															<span className='font-semibold text-gray-900'>
																{commentUser?.fullName || 'Người dùng'}
															</span>
															<Badge className={getDifficultyColor(comment.examRating)} variant='outline'>
																{getDifficultyText(comment.examRating)}
															</Badge>
														</div>
				...
				<div className='flex items-center gap-4 pt-4'>
					...
					{activeTab === 'practice' && selectedSectionIds.length > 0 && (
						<span className='text-sm text-gray-600'>Đã chọn {selectedSectionIds.length} phần thi</span>
					)}
				</div>
			</div>
